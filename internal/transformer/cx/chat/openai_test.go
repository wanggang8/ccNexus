...
func TestOpenAITransformer_TransformResponseWithContext_ClaudeToolUseStartShouldEmitToolCallsImmediately(t *testing.T) {
	trans := NewOpenAITransformer("gpt-4")
	ctx := transformer.NewStreamContext()
...
`)
	...
	_, payload := convert.ParseSSE(second)
	if payload == "" {
		t.Fatalf("expected SSE payload for tool_start, got %q", string(second))
	}
	var chunk map[string]interface{}
	if err := json.Unmarshal([]byte(payload), &chunk); err != nil {
		t.Fatalf("failed to unmarshal tool_start payload: %v", err)
	}

	choices, ok := chunk["choices"].([]interface{})
	if !ok || len(choices) == 0 {
		t.Fatalf("expected choices in tool_start chunk, got %#v", chunk["choices"])
	}
	delta, ok := choices[0].(map[string]interface{})["delta"].(map[string]interface{})
	if !ok {
		t.Fatalf("expected delta object in tool_start chunk, got %#v", choices[0])
	}
	toolCalls, ok := delta["tool_calls"].([]interface{})
	if !ok || len(toolCalls) == 0 {
		t.Fatalf("expected tool_calls in tool_start chunk, got %#v", delta)
	}
	toolCall := toolCalls[0].(map[string]interface{})
	function := toolCall["function"].(map[string]interface{})
	if function["name"] != "read_file" {
		t.Fatalf("expected tool call name read_file, got %#v", function["name"])
	}
	if function["arguments"] != "" {
		t.Fatalf("expected empty tool arguments on tool start, got %#v", function["arguments"])
	}
}
